...
struct neural_net
{
  static constexpr neurons_per_layer = layer_blocks * 16;
  static constexpr weights_per_neuron = neurons_per_layer;
  static constexpr weights_per_layer = neurons_per_layer * neurons_per_layer;
  static constexpr biases_per_layer = neurons_per_layer;
...
{
  LS_ALIGN(32) int8_t data[layer_blocks * 16];
};
...
{
  LS_ALIGN(32) int16_t tmp[layer_blocks * 16];;
  ...
  const __m128i _min_16 = _mm256_set1_epi8(lsMinValue<int8_t>());
  const __m128i _max_16 = _mm256_set1_epi8(lsMaxValue<int8_t>());
  ...
  for (size_t layer = 0; layer < layers; layer++)
  {
    ...
    for (size_t neuron = 0; neuron < nn.neurons_per_layer; neuron++)
    {
      for (size_t inputBlock = 0; inputBlock < layer_blocks; inputBlock++)
      {
        ...
        const __m128i weightLo = _mm_load_si128(pLayer);
        const __m128i weighsHi = _mm_load_si128(pLayer + 1);
        pLayer++;

        const __m128i *pOut128 = reinterpret_cast<const __m128i *>(pOut + inputBlock);
        const __m128i inLo = _mm256_load_si256(pOut128);
        const __m128i inHi = _mm256_load_si256(pOut128 + 1);
        ...
        const __m256i resLo16 = _mm256_sra_epi16(res16Lo16, 7);
        const __m256i resHi16 = _mm256_sra_epi16(res16Hi16, 7);
    ...
    for (size_t inputBlock = 0; inputBlock < layer_blocks; inputBlock++)
    {
      ...
      const __m256i sum8 = _mm256_set_m128(sumHi8, sumLo8);
